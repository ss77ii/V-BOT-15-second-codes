...
void odometry_fn(void *param)
{
  ...
  while (true)
  {
    ...
    cur_left_wheel_encoder = (left_front_motor.get_position() + left_mid_motor.get_position() + left_back_motor.get_position()) / 3.0;
    cur_right_wheel_encoder = (right_front_motor.get_position() + right_mid_motor.get_position() + right_back_motor.get_position()) / 3.0;
    ;
    ...
    if ((pre_cur_X_diff != 0) || (pre_cur_Y_diff != 0))
    {
      ...
      else
      {
        ...
        else if (pre_cur_X_diff < 0 && pre_cur_Y_diff >= 0)
      ...
      if ((left_wheel_encoder_diff > 0 && right_wheel_encoder_diff > 0) || (left_wheel_encoder_diff < 0 && right_wheel_encoder_diff < 0))
      {
...
/*
void odometry_fn_old(void* param){  //add some functions that use position and heading to find the x and y coord necessary to grab a goal or smth

  //loop interval variables
  double loopStartTime;
  double loopEndTime;
  double loopTime; //loop time spent so far
  double loopDuration = 5; //millis

  double currentHeading = get_robot_heading_radians_lib(hardwareParameter);
  double previousHeading = get_robot_heading_radians_lib(hardwareParameter);

  double previousEncoderValue = Y_encoder.get_value();
  double currentEncoderValue = previousEncoderValue;
  double differenceEncoderValue = 0;

  double differenceX;
  double differenceY;

  //double previousEncoderX = sys_encoderPosition.x;
  //double previousEncoderY = sys_encoderPosition.y;

  double previousLeftWheelEncoder = 0;
  double previousRightWheelEncoder = 0;
  double currentLeftWheelEncoder;
  double currentRightWheelEncoder;

  double leftWheelEncoderDifference;
  double rightWheelEncoderDifference;

  Point preCoodinate = get_coordinate();
  Point curCoodinate = get_coordinate();

  double avgHeading;

  while (true){
    loopStartTime = pros::millis();

    currentHeading = get_robot_heading_radians_lib(hardwareParameter);
    currentEncoderValue = Y_encoder.get_value();
    differenceEncoderValue = currentEncoderValue - previousEncoderValue;
    currentLeftWheelEncoder = (left_front_motor.get_position() + left_mid_motor.get_position()
                               + left_back_motor.get_position()) / 3.0;
    currentRightWheelEncoder = (right_front_motor.get_position() + right_mid_motor.get_position()
                                + right_back_motor.get_position()) / 3.0;

    if (differenceEncoderValue == 0){
      no_action_num++;
      //do nothing
    }
    else{
      leftWheelEncoderDifference = currentLeftWheelEncoder - previousLeftWheelEncoder;
      rightWheelEncoderDifference = currentRightWheelEncoder - previousRightWheelEncoder;

      if((leftWheelEncoderDifference > 0 && rightWheelEncoderDifference < 0) ||
        (leftWheelEncoderDifference < 0 && rightWheelEncoderDifference > 0)){//rotation
        double diff = (leftWheelEncoderDifference + rightWheelEncoderDifference)/2.0 * 4 / 3.0;
        avgHeading = (currentHeading + previousHeading) / 2.0;
        differenceX = diff * cos(avgHeading);
        differenceY = diff * sin(avgHeading);

        curCoodinate.x = preCoodinate.x + degreeToDistanceCM(differenceX, WHEEL_CIRCUMFERENCE_CM + 0.4);
        curCoodinate.y = preCoodinate.y + degreeToDistanceCM(differenceY, WHEEL_CIRCUMFERENCE_CM + 0.4);
        update_coordinate(curCoodinate);
        wheel_num++;
      }
      else{
        double ratio;
        double enlarge = 0.75;
        avgHeading = (currentHeading + previousHeading) / 2.0;
        if(rightWheelEncoderDifference == 0){
          ratio = 10;
        } else {
          ratio = leftWheelEncoderDifference/rightWheelEncoderDifference;
        }

//        if(ratio > 1.05){
//          differenceX = differenceEncoderValue * cos(avgHeading)/enlarge;
//          differenceY = differenceEncoderValue * sin(avgHeading)/enlarge;
//        } else if(ratio < 1.05){
//          differenceX = differenceEncoderValue * cos(avgHeading)*enlarge;
//          differenceY = differenceEncoderValue * sin(avgHeading)*enlarge;
//        } else {

        differenceX = differenceEncoderValue * cos(avgHeading);
        differenceY = differenceEncoderValue * sin(avgHeading);

        curCoodinate.x = preCoodinate.x
              + degreeToDistanceCM(differenceX, ENCODER_CIRCUMFERENCE_CM + 0.3);
        curCoodinate.y = preCoodinate.y
              + degreeToDistanceCM(differenceY, ENCODER_CIRCUMFERENCE_CM + 0.3);
        update_coordinate(curCoodinate);
        encoder_num++;
      }
    }

    previousLeftWheelEncoder = currentLeftWheelEncoder;
    previousRightWheelEncoder = currentRightWheelEncoder;
    previousHeading = currentHeading;
    previousEncoderValue = currentEncoderValue;
    preCoodinate.x = curCoodinate.x;
    preCoodinate.y = curCoodinate.y;

    loopEndTime = pros::millis();
    loopTime = loopEndTime - loopStartTime;
    if (loopTime > 0){
      delay(loopDuration - loopTime);
    } else{
      delay(loopDuration);
    }
  }
